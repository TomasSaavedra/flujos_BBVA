...
var s_account="bbvachilenewdev,bbvachilenewprod";
...
if(window.location.href.indexOf("bbvanet") > -1){
  s.abort = "true";
}
...
s.linkInternalFilters="javascript:,bbva.cl,ei-web-chile.igrupobbva:58081,de-web-chile.igrupobbva:58081" //optional: add your internal domain here
...
var url = window.location.href;
	var termino = "";
	var iniTermino = url.indexOf("q=");
	for(var contadorTermino = iniTermino+2; contadorTermino < url.length; contadorTermino++){
		var caracterTermino = url.charAt(contadorTermino);
		if(caracterTermino != "&"){
			termino += caracterTermino;
		}else{
			break;
		}
	}
	var lastUrl = s.getValOnce(termino, "s_last");
	window.lastUrl = lastUrl;
...
var cont = 0;
  if(s.pageName === undefined){
    ...
    for(var i = 0; i < s.pageName.length; i++){
	
      if(s.pageName.charAt(i) == ":"){
        if(cont > 1){
          break;
        }
        cont++;
