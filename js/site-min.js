$(function() {
   ...
   /* $options.css("width", 90 / $options.length + "%");
    ...
    var initOperationContainer = function($context) {
        ...
        $(".close-operation-tab", $context).each(function() {
            $(this).on("vclick", function() {
                if (Modernizr.mq("(max-width:768px)")) {
                    //alert('cerrando');
					if($("#tab-operaciones-link-virtual").length >= 1)
						{
							$("#tab-operaciones-link-virtual").remove();
    ...
    var scrollPaneOptions = {verticalGutter: 0,autoReinitialiseDelay: 0,showArrows: true,contentWidth: 132,verticalArrowPositions: "after",horizontalArrowPositions: "after"};
    ...
    var windowResize = function() {
        $(window).on("throttledresize", function() {
			...
			var $wizard = $("#operacion-consulta, #option-content, #operacion-container").find(".wizard");
            if ($wizard.length > 0) {
                    $(document).trigger("capgemini.wizard.resize", $wizard);
					//console.log("resize wizard");
            }
			
			if($("#operacion-consulta, #option-content, #operacion-container").text().length > 1)
			{
